#!/usr/bin/env node
// puppet installer — clone repo + create Claude Code skill symlink
...
const REPO = 'https://github.com/EngineeredReiwa/x-puppet.git';
const INSTALL_DIR = path.join(os.homedir(), 'puppet');
const SKILL_SOURCE = path.join(INSTALL_DIR, '.claude', 'skills', 'x-puppet');
const SKILL_TARGET = path.join(os.homedir(), '.claude', 'skills', 'puppet');

console.log('🎭 puppet installer\n');
...
if (fs.existsSync(SKILL_TARGET)) {
  ...
  if (stat.isSymbolicLink()) {
    ...
    console.log('⚠️  ~/.claude/skills/puppet already exists (not a symlink). Skipping.');
  }
}

if (!fs.existsSync(SKILL_TARGET)) {
  fs.symlinkSync(SKILL_SOURCE, SKILL_TARGET);
  console.log('🔗 Skill linked: ~/.claude/skills/puppet');
}
...
Next steps:
  ...
  3. Use it:
     node ~/puppet/puppet.js x timeline 5
     node ~/puppet/puppet.js reddit feed 10
